/**
 * Mirrors ml/schemas.py's RiskResult contract and rag_routes.py's
 * RagQueryResponse contract — the mobile app's copy of the same shapes
 * frontend/src/types.ts keeps in sync with the backend. Field names must
 * stay disease-agnostic; nothing here may name a specific condition.
 */
...
/** Extended driver info for models that can cite a source per factor (ml/schemas.py's DriverDetail). */
...
/** Frontend copy of ml/rag_routes.py's RagQueryResponse contract. */
...
/** Mirrors ml/schemas.py's PatientAppContent — content backing the home/ask screens. */
